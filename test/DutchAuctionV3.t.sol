...
contract DutchAuctionV3Test is Test {
    Auction auction;
    CustomERC20 token;
    ...
    function createAuctionV3Ether(bool _devFeeInToken) public {
    ...
    function testCreateAuctionV3Ether1() public {
        createAuctionV3Ether(false);
    ...
    function testCreateAuctionV3Ether2() public {
        createAuctionV3Ether(false);
        vm.warp(1 seconds);
        vm.prank(xlazer);
        ...
        assertEq(token.balanceOf(xlazer), 5 * 1e8);
        assertEq(xlazer.balance, 5 ether);

        vm.prank(xlazer2);
        auction.claimTokens(0);
        assertEq(token.balanceOf(xlazer2), 3 * 1e8);
    ...
    function testCreateAuctionV3Ether3() public {
        createAuctionV3Ether(true);
        vm.warp(1 seconds);
        vm.prank(xlazer);
        auction.buyToken{value: 10 ether}(0, 10e18);
        ...
        vm.prank(xlazer2);
        auction.claimTokens(0);
        assertEq(token.balanceOf(xlazer2), 5 * 1e8);
    ...
    function createAuctionV3PT(bool _devFeeInToken) public {
    ...
    function testCreateAuctionV3PT1() public {
        createAuctionV3PT(true);
        vm.prank(alice);
    ...
    function testCreateAuctionV3PT2() public {
        createAuctionV3PT(false);
        vm.startPrank(alice);
        auction.whitelistAddress(0, xlazer);
        ...
        vm.prank(xlazer);
        auction.claimTokens(0);
        assertEq(token.balanceOf(xlazer), 1 * 1e8);
        assertEq(purchaseToken.balanceOf(xlazer), 5 * 1e18);

        vm.warp(2 hours + 30 days);
        vm.prank(xlazer);
        auction.claimTokens(0);
        assertEq(token.balanceOf(xlazer), 1.5 * 1e8);
    ...
    //test for alt fee option
    function testCreateAuctionV3PT3() public {
        createAuctionV3PT(true);
        vm.startPrank(alice);
        auction.whitelistAddress(0, xlazer);
        auction.whitelistAddress(0, xlazer2);
        ...
        vm.prank(xlazer2);
        auction.claimTokens(0);
        assertEq(token.balanceOf(xlazer2), 1 * 1e8);
        assertEq(purchaseToken.balanceOf(xlazer2), 0);
