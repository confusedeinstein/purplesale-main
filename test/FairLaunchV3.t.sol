...
contract FairLaunchTestV3 is Test {
    ...
    function createLaunchV3Ether(
        ...
        vm.startPrank(alice);
        token.mint(alice, 6 * 1e8);
        if (_devFeeInToken) token.mint(alice, 0.2 * 1e8);
    ...
    function testCreateLaunchV3Ether1() public {
        createLaunchV3Ether(0, false);
    ...
    function testCreateLaunchV3Ether2() public {
        ...
        assertGt(token.balanceOf(xlazer), 3.3 * 1e8);

        vm.prank(xlazer2);
        fairLaunch.claimTokens(0);
        assertGt(token.balanceOf(xlazer2), 6.6 * 1e8);
    ...
    function testCreateLaunchV3Ether3() public {
        createLaunchV3Ether(5, true);
        vm.warp(2 hours);
        vm.startPrank(xlazer);
        fairLaunch.buyToken{value: 1 ether}(0, 1e18, address(0));
        ...
        assertEq(token.balanceOf(bob), 0.2 * 1e8);
        vm.stopPrank();
    ...
    function testCreateLaunchV3PT1() public {
        createLaunchV3PT(0, true);
        vm.prank(alice);
        fairLaunch.whitelistAddress(0, xlazer);

        vm.startPrank(xlazer);
        vm.warp(2 hours);
        purchaseToken.approve(address(fairLaunch), 1 * 1e18);
        fairLaunch.buyToken(0, 1e18, address(0));
        ...
        vm.prank(alice);
        fairLaunch.refundInvestment(0);
        assertEq(token.balanceOf(alice), 16.2e8);
    }

    // test for whitelisting, multiple user buying, claiming tokens, handle after sale & dev commision
    function testCreateLaunchV3PT2() public {
        createLaunchV3PT(0, false);
        vm.startPrank(alice);
        fairLaunch.whitelistAddress(0, xlazer);
        ...
        vm.warp(2 hours);
        vm.startPrank(xlazer);
        purchaseToken.approve(address(fairLaunch), 4 * 1e18);
        fairLaunch.buyToken(0, 4e18, address(0));
        purchaseToken.transfer(xlazer2, 5e18);
        vm.stopPrank();

        vm.startPrank(xlazer2);
        purchaseToken.approve(address(fairLaunch), 5 * 1e18);
        fairLaunch.buyToken(0, 5e18, address(0));
        ...
        vm.prank(xlazer);
        fairLaunch.claimTokens(0);
        assertGt(token.balanceOf(xlazer), 4.4 * 1e8);

        vm.warp(2 weeks + 30 days);
        vm.prank(xlazer2);
        fairLaunch.claimTokens(0);
        assertGt(token.balanceOf(xlazer2), 5.5 * 1e8);
    ...
    //test for affiliate commission & alt fee option
    function testCreateLaunchV3PT3() public {
        createLaunchV3PT(5, true);
        vm.prank(alice);
        fairLaunch.whitelistAddress(0, xlazer);

        vm.warp(2 hours);
        vm.startPrank(xlazer);
        purchaseToken.approve(address(fairLaunch), 10 * 1e18);
        fairLaunch.buyToken(0, 4e18, address(0));
        fairLaunch.buyToken(0, 6e18, xlazer2);

        vm.warp(2 weeks + 240 days);
        fairLaunch.claimTokens(0);
        assertEq(token.balanceOf(xlazer), 10 * 1e8);
        vm.stopPrank();
