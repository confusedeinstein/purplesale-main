...
contract PresaleTestV3 is Test {
    Presale presale;
    CustomERC20 token;
    ...
    function setUp() public virtual {
        ...
        deal(alice, 10 ether);
        vm.prank(alice);
        token = new CustomERC20("Test Token", "TT", 6, 10);
    ...
    function createPresaleV3Ether(
        ...
        vm.startPrank(alice);
        if (_devFeeInToken) token.mint(alice, 0.2 * 1e6);
    ...
    function testCreatePresaleV3Ether1() public {
        createPresaleV3Ether(0, false);
    ...
    function testCreatePresaleV3Ether2() public {
        createPresaleV3Ether(0, false);
        vm.warp(2 hours);
        vm.prank(xlazer);
        presale.buyToken{value: 1 ether}(0, 1e18, address(0));
        ...
        assertEq(token.balanceOf(xlazer), 2 * 1e6);

        vm.prank(xlazer2);
        presale.claimTokens(0);
        assertEq(token.balanceOf(xlazer2), 4 * 1e6);
    ...
    function testCreatePresaleV3Ether3() public {
        createPresaleV3Ether(5, true);
        vm.warp(2 hours);
        vm.startPrank(xlazer);
        presale.buyToken{value: 1 ether}(0, 1e18, address(0));
        ...
        vm.warp(2 weeks);
        presale.claimTokens(0);
        assertEq(token.balanceOf(xlazer), 6e6);
        vm.stopPrank();
        ...
        assertEq(token.balanceOf(bob), 0.12 * 1e6);
        vm.stopPrank();
    ...
    function testFailcreatePresaleV3Ether() public {
        createPresaleV3Ether(0, false);
        vm.startPrank(xlazer);
        vm.warp(1 + 1 days);
    ...
    function testCreatePresaleV3PT1() public {
        createPresaleV3PT(0, true);
        vm.prank(alice);
        ...
        vm.prank(alice);
        presale.refundInvestment(0);
        assertEq(token.balanceOf(alice), 10.2e6);
    }

    // test for whitelisting, multiple user buying, claiming tokens (vesting), handle after sale & dev commision
    function testCreatePresaleV3PT2() public {
        createPresaleV3PT(0, false);
        vm.startPrank(alice);
        presale.whitelistAddress(0, xlazer);
        ...
        vm.prank(xlazer);
        presale.claimTokens(0);
        assertEq(token.balanceOf(xlazer), 0.8 * 1e6);

        vm.warp(2 weeks + 30 days);
        vm.prank(xlazer);
        presale.claimTokens(0);
        assertEq(token.balanceOf(xlazer), 1.2 * 1e6);

        vm.warp(2 weeks + 240 days);
        vm.prank(xlazer);
        presale.claimTokens(0);
        assertEq(token.balanceOf(xlazer), 4 * 1e6);

        vm.warp(2 weeks + 240 days);
        vm.prank(xlazer2);
        presale.claimTokens(0);
        assertEq(token.balanceOf(xlazer2), 5 * 1e6);

        vm.prank(alice);
        presale.handleAfterSale(0);
        assertEq(purchaseToken.balanceOf(alice), 8.55 * 1e18);
        assertEq(token.balanceOf(alice), 1 * 1e6);
    ...
    //test for affiliate commission & alt fee option
    function testCreatePresaleV3PT3() public {
        createPresaleV3PT(5, true);
        vm.prank(alice);
        presale.whitelistAddress(0, xlazer);
        ...
        vm.warp(2 weeks + 240 days);
        presale.claimTokens(0);
        assertEq(token.balanceOf(xlazer), 10 * 1e6);
        vm.stopPrank();
        ...
        assertEq(token.balanceOf(bob), 0.2 * 1e6);
        vm.stopPrank();
...
//purchase token - decimals can be anything
