...
contract PresaleTestV2 is Test {
    Presale presale;
    CustomERC20 token;
    ...
    function testCreatePresaleV2Ether1() public {
        createPresaleV2Ether(0, false);
    ...
    function testCreatePresaleV2Ether2() public {
        createPresaleV2Ether(0, false);
        vm.warp(2 hours);
        vm.prank(xlazer);
        presale.buyToken{value: 1 ether}(0, 1e18, address(0));
    ...
    function testCreatePresaleV2Ether3() public {
        createPresaleV2Ether(5, true);
        vm.warp(2 hours);
        vm.startPrank(xlazer);
        presale.buyToken{value: 1 ether}(0, 1e18, address(0));
    ...
    function testFailcreatePresaleV2Ether() public {
        createPresaleV2Ether(0, false);
        vm.startPrank(xlazer);
        vm.warp(1 + 1 days);
    ...
    function createPresaleV2PT(
        ...
        uint256 _hardCap = 10 * 10 ** 8;
        uint256 _presaleRate = 1;
        uint256 _minBuy = 1 * 10 ** 8;
    ...
    function testCreatePresaleV2PT1() public {
        createPresaleV2PT(0, true);
        vm.prank(alice);
        presale.whitelistAddress(0, xlazer);

        vm.startPrank(xlazer);
        vm.warp(2 hours);
        purchaseToken.approve(address(presale), 1 * 1e8);
        presale.buyToken(0, 1e8, address(0));
    ...
    function testCreatePresaleV2PT2() public {
        createPresaleV2PT(0, false);
        vm.startPrank(alice);
        presale.whitelistAddress(0, xlazer);
        ...
        vm.warp(2 hours);
        vm.startPrank(xlazer);
        purchaseToken.approve(address(presale), 4 * 1e8);
        presale.buyToken(0, 4e8, address(0));
        purchaseToken.transfer(xlazer2, 5e8);
        vm.stopPrank();

        vm.startPrank(xlazer2);
        purchaseToken.approve(address(presale), 5 * 1e8);
        presale.buyToken(0, 5e8, address(0));
    ...
    //test for affiliate commission & alt fee option
    function testCreatePresaleV2PT3() public {
        createPresaleV2PT(5, true);
        vm.prank(alice);
        presale.whitelistAddress(0, xlazer);

        vm.warp(2 hours);
        vm.startPrank(xlazer);
        purchaseToken.approve(address(presale), 10 * 1e8);
        presale.buyToken(0, 4e8, address(0));
        presale.buyToken(0, 6e8, xlazer2);
