...
contract SubscriptionTestV2 is Test {
    Subscription sub;
    CustomERC20 token;
    ...
    function createSubV2Ether(bool _devFeeInToken) public {
        bool _whitelistedEnabled = false;
        uint256 _softCap = 500 * 10 ** 18;
        uint256 _hardCap = 1000 * 10 ** 18;
        uint256 _hardCapPerUser = 250 * 10 ** 18;
        ...
        vm.startPrank(alice);
        token.mint(alice, 996 * 1e18);
        if (_devFeeInToken) token.mint(alice, 20 * 1e18);
    ...
    function testCreateSubV2Ether1() public {
        createSubV2Ether(false);
        ...
        vm.prank(alice);
        sub.refundInvestment(0);
        assertEq(token.balanceOf(alice), 1006 * 1e18);
    }

    //test for multiple user buying, claiming tokens, handle after sale, dev commision
    function testCreateSubV2Ether2() public {
        createSubV2Ether(false);
        vm.warp(2 hours);
        vm.prank(xlazer);
        ...
        sub.claimTokens(0);
        assertEq(token.balanceOf(xlazer), 250 * 1e18);
        assertEq(xlazer.balance, 7.5 ether);
        vm.stopPrank();

        vm.prank(xlazer2);
        sub.claimTokens(0);
        assertEq(token.balanceOf(xlazer2), 250 * 1e18);

        vm.prank(xlazer3);
        sub.claimTokens(0);
        assertEq(token.balanceOf(xlazer3), 250 * 1e18);

        vm.prank(xlazer4);
        sub.claimTokens(0);
        assertEq(token.balanceOf(xlazer4), 250 * 1e18);
    ...
    function testCreateSubV2Ether3() public {
        createSubV2Ether(true);
        vm.warp(2 hours);
        vm.prank(xlazer);
        sub.buyToken{value: 3 ether}(0, 3e18);
    ...
    function createSubV2PT(bool _devFeeInToken) public {
    ...
    function testCreateSubV2PT1() public {
        createSubV2PT(true);
        vm.prank(alice);
        ...
        vm.startPrank(xlazer);
        vm.warp(2 hours);
        purchaseToken.approve(address(sub), 1 * 1e8);
        sub.buyToken(0, 1e8);
        ...
        vm.prank(alice);
        sub.refundInvestment(0);
        assertEq(token.balanceOf(alice), 1026 * 1e18);
    }

    //test for multiple user buying, claiming tokens, handle after sale, dev commision
    function testCreateSubV2PT2() public {
        createSubV2PT(false);
        vm.startPrank(alice);
        sub.whitelistAddress(0, xlazer);
        ...
        vm.warp(2 hours);
        vm.startPrank(xlazer);
        purchaseToken.approve(address(sub), 3 * 1e8);
        sub.buyToken(0, 3e8);
        purchaseToken.mint(xlazer2, 1e8);
        purchaseToken.mint(xlazer3, 0.5e8);
        purchaseToken.mint(xlazer4, 0.5e8);
        vm.stopPrank();
        ...
        purchaseToken.approve(address(sub), 0.5 * 1e8);
        sub.buyToken(0, 0.5e8);
        vm.stopPrank();
        ...
        sub.claimTokens(0);
        assertEq(token.balanceOf(xlazer), 250 * 1e18);
        assertEq(purchaseToken.balanceOf(xlazer), 7.5 * 1e8);
        vm.stopPrank();
        ...
        vm.prank(alice);
        sub.handleAfterSale(0);
        assertEq(purchaseToken.balanceOf(alice), 4.275 * 1e8);

        vm.prank(bob);
        sub.collectDevCommission(0);
        assertEq(purchaseToken.balanceOf(bob), 0.225 * 1e8);
    }

    //test for alt fee option
    function testCreateSubV2PT3() public {
        createSubV2PT(true);
        vm.startPrank(alice);
        sub.whitelistAddress(0, xlazer);
        sub.whitelistAddress(0, xlazer2);
        ...
        vm.prank(alice);
        sub.handleAfterSale(0);
        assertEq(purchaseToken.balanceOf(alice), 4.41 * 1e8);

        vm.prank(bob);
        sub.collectDevCommission(0);
        assertEq(purchaseToken.balanceOf(bob), 0.09 * 1e8);
        assertEq(token.balanceOf(bob), 20 * 1e18);
    }
}
